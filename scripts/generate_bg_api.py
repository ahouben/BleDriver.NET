...
class BgApiCommandEvent(object):
    ...
    def return_struct(self):
        return (self.returns and f'Task<{self.struct()}>') or 'Task'
    def struct(self):
...
''' % {
        ...
        'return' : (self.isEvent and 'void') or f'async {self.return_struct()}',
    ...
    def dumpBody(self):
        ...
        s += '''
            // send
            %(response)sawait Send(new BgApiCommand() { Data = _data }, %(no_return)s);''' % { 'response': (self.returns and 'BgApiResponse response = ') or '',
                                                                                         'no_return': (self.no_return and 'true') or 'false' }
        if self.returns:
            s += '''
            return (%(struct)s)response;''' % { 'struct' : self.struct() }
        return s
...
if __name__ == '__main__':
    bg_api(os.path.join(_dir, 'ble-1.10.0-153', 'bleapi.xml'), os.path.join(_dir, '..', 'src', 'BgApi.g.cs'))
